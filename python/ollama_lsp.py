...
class OllamaServer:
    ...
    def on_initialize(self, params: types.InitializeParams):
        ...
        requests.post("http://localhost:8000",headers=headers, json={"message": "Initialized the ollama LSP server", 'init options' : str(params.capabilities), "time":  time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())})
        return {
            "capabilities": {
    ...
    def on_change(self, params: types.DidChangeTextDocumentParams):
        headers = {'Content-type': 'application/json'}
        requests.post("http://localhost:8000",headers=headers, json={"message": "Document changed","curr_suggestion" : self.curr_suggestion, "changes" :str(params.content_changes)})
        change = params.content_changes[0]
        if change.text == self.curr_suggestion['suggestion'][0:len(change.text)]: # If the change is a part of the suggestion, remove that part from the suggestion
